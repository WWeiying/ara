# Standalone 1 GHz synthesis check for the QBS activation-context block.
...
set output_dir [file normalize [file join [pwd] qbs_activation_context_dc_out]]
...
analyze -format sverilog -define {SYNTHESIS TARGET_SYNTHESIS TARGET_SRAM_MC} [list \
  [file join $ara_root hardware/include/qbs_pkg.sv] \
  [file join $ara_root hardware/src/vlsu/qbs/qbs_activation_context.sv]]
elaborate qbs_activation_context
current_design qbs_activation_context
link
...
write -format verilog -hierarchy -output [file join $output_dir qbs_activation_context.v]
exit
