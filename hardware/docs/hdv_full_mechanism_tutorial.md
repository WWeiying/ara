...
`req_pending_q` 表示 IPU 已经发出一条 read request 但还没收到 response。当前 IPU 一次只允许一个 outstanding fetch request。
...
`ipu_mem_req_addr_o = fetch_base_q + fill_idx_q * PacketBytes`。

`fetch_base_q` 指向当前 fill buffer 的起始地址。`fill_idx_q` 是这个 buffer 内第几个 16B packet。
...
VLIWPU 内部只有一个 `packet_hold_valid_q`。当没有持有 packet 时，`vliwpu_ipu_packet_ready_o=1`，可以从 IPU 接收一个 fetch packet。
...
`slot_found/slot_idx` 每周期从低到高找第一个 `insn_valid_q[i]=1` 的 slot。这样 vector EP 内指令按 slot 顺序发给 Ara。

### 8.5 request 生成

只有在 `state_q == DISPATCH && slot_found` 时，模块驱动：

- `acc_req.req_valid=1`
- `acc_req.insn = insn_q[slot_idx]`
- `acc_req.frm = RNE`
- `acc_req.rs1/rs2 = vtrace 当前项`

如果 vtrace 耗尽或当前 vtrace 指令与 EP 指令不匹配，则 `req_valid=0` 并报错，避免把错误标量值发给 Ara。
...
因此只要 Ara `req_ready` 连续为 1，该模块可以连续每周期发送当前 EP 内的下一条向量指令。
...
- ar_valid = IPU req valid 且没有 read inflight。
- ar.addr = `ipu_mem_req_addr`。
- ar.len = 0，单 beat。
- ar.size = log2(16B)，对应 128-bit fetch packet。
- r_ready = read inflight 且 IPU ready 接收 response。

`imem_read_inflight_q` 在 AR handshake 后置 1，在 R handshake 且 `last` 后清 0。dispatch flush 也清 inflight，避免 redirect 后旧 response 继续被当成有效取指。
