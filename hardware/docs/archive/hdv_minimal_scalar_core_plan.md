...
→ 论文明确允许标量侧做简化实现。本文早期方案曾把"五级双发射"降级为
**单发射、按 EP 切片顺序执行的最小标量核**。当前实现已进一步演进为轻量 3 发射：
2 条 simple ALU lane 加 1 条 complex lane，但仍保留共享寄存器堆这一核心语义。
...
| `cva6_hdv_scalar_backend` | 复用 CVA6 `compressed_decoder`、`decoder`、`alu`、`branch_unit`、`mult`、`fpu_wrap`，轻量 3 发射执行 scalar slots，维护 XRF/FRF/CSR stub |
| `hdv_vec_dispatch_unit` | 将 vector slots 串行发送到 Ara；真实标量模式下通过 operand service 读取 XRF/FRF；scalar-visible vector response 写回标量后端 |
...
- 当前路径更直接：vector EP 由 `hdv_vec_dispatch_unit` 送 Ara；标量寄存器操作数通过 operand service 从 `cva6_hdv_scalar_backend` 读取；`vset rd`、`vmv.x.s`、`vfmv.f.s` 等 scalar-visible vector response 通过 `vec_wb_*` 写回。
...
- 为保证真实标量寄存器值正确，`hdv_vec_dispatch_unit` 在 `UseVTraceScalar=0` 时不会 EP 入队即 accepted，而是等本 EP 的 vector slot 都被 dispatch FSM 消费、标量操作数已进入 Ara request 或 resolved command window 后 accepted；`vset rd!=0` 还要等 response 写回。
