...
深度学习、科学计算等数据并行应用持续提高对处理器吞吐、能效和可扩展性的需求。RISC-V 向量扩展（RVV）以向量长度无关模型支持可伸缩数据并行执行，但主流紧耦合实现仍由标量核逐条驱动向量后端，在短向量、循环尾部和控制/访存交织阶段受到前端供给、细粒度同步和保守后端调度限制。本文提出 **SEAM-V**，一种面向 RVV 的混合解耦向量执行架构，其核心是在前端供给、向量派发与后端执行之间保持 execute packet（EP）语义连续性。SEAM-V 将热点向量 kernel 从标量核逐条发射中解耦，并通过本地指令供给、标量控制和 VLIW 风格打包形成连续 EP 流；同时借助轻量级 HINT 标注表达包边界、局部并行性、循环状态和访存意图，并将 EP 上下文随向量请求传递至后端。基于后端可见 EP，SEAM-V 在保持 RVV ISA、软件模型和后端动态正确性边界不变的前提下，协同优化相关性处理、规则访存提前化和跨 EP 执行重叠，从而提升标量控制、向量计算与访存访问之间的流水并行度。基于端到端 RTL 实现和物理级评估，SEAM-V 相比基线在 [待填 workload/benchmark] 上实现 [待填] 的性能提升和 [待填] 的能效收益，面积和功耗代价分别为 [待填] 和 [待填]。

## 1. Introduction（引言）

深度神经网络、科学计算和自动驾驶等数据并行负载持续推动通用处理器向向量化架构演进。RISC-V 向量扩展（RVV）以开放、可伸缩和向量长度无关的编程模型，为不同物理向量长度下的软件可移植性与性能扩展提供统一接口 [1]。现有 RVV 处理器通常采用紧耦合执行组织：应用级标量核负责取指、译码、控制流、地址计算和向量指令发射，向量单元作为后端执行数据并行命令。该组织接口清晰、兼容性好，但执行效率受限于标量驱动的逐条命令流模型：向量后端虽具备较强的数据并行能力，前端供给、控制推进、后端调度和访存访问之间却缺乏能够跨前后端保留的包级执行语义。

这一限制会在不同粒度和不同阶段的实际负载中显现。短向量、循环尾部、小 batch 算子和小矩阵计算会削弱向量指令对标量控制开销的摊销能力；控制密集、不规则或访存敏感 kernel 则会因标量/向量交织、保守相关性处理和内存等待破坏向量执行连续性。即使应用整体具有较大数据并行度，其内部也常包含向量粒度、控制强度和访存行为不断变化的执行阶段。因此，问题并不局限于短 AVL 场景，而是源于传统逐条供给模型难以向后端持续传递可用于调度和访存协同的包级执行上下文。

已有方法主要从增强前端、解耦执行和显式并行表达等方向提升向量执行效率。一类方案通过更强的标量主核、乱序发射或复杂内存系统提高指令投放速率和延迟容忍能力，但通常伴随更高的面积、功耗和验证成本 [2]–[4]。另一类方案采用解耦执行或 VLIW 风格组织，使向量侧减少对标量核逐条同步发射的依赖，并在前端形成更连续的指令供给 [5]–[8]。然而，前端供给连续性并不等价于端到端执行效率：若前端形成的包级并行性、控制边界和访存意图无法跨越后端接口保留下来，向量后端仍只能消费无包级语义的命令流，并以保守方式处理相关性、访存请求和执行调度。包级执行语义在前后端之间的退化，成为限制 RVV 向量处理器持续吞吐提升的关键缺口。

本文提出 **SEAM-V**，一种面向 RVV 的混合解耦向量执行架构。其核心是以 execute packet（EP）贯通前端供给与后端执行：前端通过任务级解耦、本地指令获取、标量控制与 VLIW 风格组织，将细粒度交织的标量/向量指令流重构为连续 EP 流；软件借助轻量级 HINT header 与 p-bit 标记包边界、局部并行性、循环属性和访存意图，使 EP 同时承载前端供给粒度和后端可消费的包级语义。由此，该架构将传统 scalar-driven RVV instruction stream 转化为 EP-driven packet stream，为前端供给、后端调度和访存协同建立统一语义基础。

进一步地，SEAM-V 将 EP 身份、请求级访存提示和循环执行状态随向量请求传播至后端，使 sequencer、VLSU 和跨包执行控制逻辑能够围绕同一 EP 上下文协同工作，支持包级相关性处理、访存提前化和受限跨包执行重叠。该机制保持 RVV 软件模型和后端正确性边界不变，同时提高标量控制、向量计算与访存访问之间的流水并行度。本文围绕这一 EP-centric 执行模型展开设计、实现与评估，验证其在不同向量粒度和执行阶段下提升持续执行效率的能力。

本文主要贡献如下：

1. **面向 RVV 的混合解耦执行架构与 Execute Packet 生成机制。** 本文提出一种前端解耦的向量执行框架，将向量任务从标量核逐条同步发射中解耦，并通过本地指令获取、标量控制与 VLIW 风格指令组织，将细粒度交织的标量/向量指令流重构为以 EP 为基本单位的连续供给结构，从而缓解多粒度和控制/访存敏感场景下的前端供给瓶颈。
2. **后端可见的执行包语义贯通机制。** 本文将 EP 从前端打包结果提升为贯穿前后端的统一执行语义，使 EP 身份、请求级访存提示和循环执行状态能够跨越解耦边界对向量后端可见，在不修改 RVV ISA 的前提下保持包级执行语义连续性，避免其在后端退化为普通命令流。
3. **基于 EP 语义驱动的后端协同执行机制。** 本文利用后端可见的 EP 语义，使后端围绕同一包级上下文协同工作，支持同 EP 相关性裁剪、访存延迟隐藏和 memory-order-safe 的跨 EP 向量提前发射，在保证依赖关系和内存定序正确性的前提下，提高向量计算、标量控制与访存访问之间的流水并行度。
4. **完整系统实现与物理级评估。** 本文完成 SEAM-V 的端到端 RTL 实现，并结合 RTL 仿真、微结构计数器、综合与物理设计流程，评估前端解耦、执行包语义贯通和后端协同优化带来的性能与能效收益，以及相应的面积和功耗代价。
...
这一时间-空间展开特性构成了 SEAM-V 的基本设计前提。执行包可以在前端表达局部并行性、控制边界和访存意图，但不能静态决定向量指令在后端的真实执行过程。真实依赖、寄存器一致性、访存返回顺序和内存定序仍必须由向量后端动态维护。因此，面向 RVV 的执行包机制不应将后端静态化为 VLIW/SIMD 式执行阵列，而应在保留后端正确性边界的前提下，将前端可知的包级语义传递给后端利用。

### 2.2 Scalar-Driven Execution in Tightly Coupled RVV Processors

现有许多 RVV 处理器采用紧耦合组织：应用级标量核负责取指、译码、控制流、地址计算和向量指令发射，向量单元逐条接收并执行向量命令。该组织接口清晰、兼容性好，但也使标量核长期处于向量指令供给的关键路径上。每条向量指令都必须先经过标量前端投放，才能进入向量后端；当 kernel 包含短向量、循环尾部、频繁 `vset`、地址更新或标量/向量交织控制时，向量指令对前端开销的摊销能力下降，后端 lane 容易因指令吞吐不足和控制推进不连续而无法持续保持高利用率。

这一瓶颈并不只是取指或译码带宽问题，也来自逐条命令接口本身。紧耦合模式按标量指令流推进，向量后端通常以单条向量命令为粒度接收工作，难以在更大范围内感知循环片段中的控制推进、地址更新、向量访存和向量计算之间的结构关系。因此，即使向量后端具备较强的数据并行执行能力，端到端效率仍可能受限于标量侧供给节奏、细粒度同步以及逐条接口带来的调度不连续性。

### 2.3 Decoupling, Packetization, and Semantic Gap

已有研究主要从增强前端、解耦执行、显式并行表达和后端动态管理等方向提升向量执行效率。一类方案采用更宽发射、乱序执行、寄存器重命名和复杂内存系统，以提高指令投放速率和延迟容忍能力 [2]–[4]。这类方案能够缓解标量侧供给压力，但通常伴随更高的面积、功耗和验证复杂度，并且其基本接口仍以逐条向量命令为中心。

另一类方案采用解耦执行，将控制、访存或向量计算从主核逐条同步发射中分离，通过队列化接口和局部执行上下文提高供给连续性 [5], [6]。与之互补，VLIW/EPIC 及部分 DSP 架构通过软件或编译期标记显式暴露指令级并行性，将可并行操作组织为执行包，以降低硬件动态依赖发现压力 [7], [8]。这些机制能够改善前端供给或表达局部并行性，但对于 RVV，执行包不能直接替代后端动态管理：向量指令会在后端展开为多拍元素流，真实执行进度、寄存器冲突、访存返回和内存顺序仍需由 sequencer、scoreboard 和访存定序逻辑维护。

因此，关键问题不只是能否更快供给指令，或能否在前端形成执行包，而是前端形成的包级信息能否跨越解耦边界并被后端消费。若 EP 身份、包边界、包内并行性、循环状态和访存意图在后端接口处退化为普通命令流，后端仍需以保守方式处理相关性、访存请求和执行调度。本文将这种前端结构化信息无法持续传递并被后端利用的问题称为前后端执行语义鸿沟。SEAM-V 关注的正是这一缺口：不是用静态打包取代后端动态管理，而是让混合解耦前端生成的 EP 在后端正确性框架内继续作为可消费的执行语义。

### 2.4 Positioning of SEAM-V

SEAM-V 与上述方向的区别在于，它不是单纯增强标量前端，也不是将 RVV 后端静态化为 VLIW/SIMD 调度结果，而是在保持 RVV ISA、软件模型和后端动态正确性边界不变的前提下，使前端形成的 EP 继续作为后端可见的包级执行语义。具体而言，SEAM-V 通过前端解耦与 VLIW 风格组织生成 EP，并将 EP 身份、循环状态和请求级访存提示传递至向量后端，使 sequencer、VLSU 和跨 EP 执行控制能够在同一包级上下文下协同工作。

相对于前期工作 [16]，本文保留其前端解耦与执行包形成思想，并将其作为 SEAM-V 的前端基础；在此基础上，本文进一步提出后端可见的 EP 语义贯通机制、EP-driven 后端协同执行机制，以及完整系统实现与物理级评估。由此，SEAM-V 区别于仅提升供给连续性的解耦设计、仅在前端暴露并行性的 packetized execution，以及仅依赖运行时推断的后端优化机制。其核心定位是在保持 RVV 动态执行模型的前提下，建立从 EP 生成、语义传递到后端消费的端到端执行路径。
...
SEAM-V 的硬件组织可概括为五类逻辑部件。首先，host 以 task 粒度提交向量 kernel，任务接口和调度逻辑将其转化为本地执行上下文。其次，本地指令供给单元从任务入口取指，并在循环阶段复用已获取的指令流，为前端打包提供稳定的局部窗口。第三，VLIW 打包单元根据 HINT header 与 p-bit 将标量/向量交织指令组织为 EP。第四，混合执行单元将 EP 分解为 scalar slice 和 vector slice，分别送入集成标量后端和向量派发路径。最后，向量派发单元将 vector slice 中的向量指令转化为后端请求，并将 EP 语义绑定到请求元数据中。
...
本地指令供给单元以 fetch packet 形式向 VLIW 打包单元提供输入。对于循环 kernel，前端根据标量后端解析出的分支重定向推进取指，并根据显式 loop marker 与分支派生的循环状态生成 loop-active context。该机制减少循环体重复取指，并为 EP 形成提供稳定、连续的局部指令窗口，使程序中的局部并行性、控制边界和访存意图能够持续暴露给硬件。
...
混合执行单元接收 EP 后，将其划分为 scalar slice 和 vector slice。Scalar slice 由集成标量后端执行，用于循环控制、分支解析、地址计算、标量寄存器更新和任务推进；vector slice 进入向量派发路径，由派发逻辑读取所需标量操作数并转换为后端请求。由于 RVV 后端会继续管理向量指令的多拍展开，前端推进不以整条向量指令完成为条件，而以 vector slice 的标量操作数已被派发逻辑捕获、必要的 `vset` 标量可见写回已完成为条件。例如，后续标量地址更新或循环控制可能依赖实际 granted `vl`，因此相关标量指令必须等待对应 `vset` 的标量可见结果。该 acknowledge 事件仅表示 vector slice 已进入后端管理域，并不表示其中的向量指令已经执行完成。

为支持 EP 粒度的流水重叠，HEU 维护 current EP 与 buffered EP 两级状态，并为包含 vector slice 的 EP 分配轻量级 `ep_id`。`ep_id` 与请求级访存提示随向量请求进入后端，使后端能够区分不同 EP 的请求归属和访存提前化意图。受限的 buffered vector early issue 允许后续 EP 的 vector-side work 在 scalar/control 约束满足且 memory ordering 安全时提前进入后端队列，而不必等待前一 EP 的所有向量事件完全完成，从而将标量控制、向量计算和访存等待在 EP 粒度上重叠。

图 3 展示 current EP、buffered EP、scalar/vector slice 和 early-issue 安全条件。该机制只允许 memory-order-safe 的跨 EP 重叠：若 current EP 含未解析分支、标量访存或其他可能影响内存顺序的操作，或者 buffered vector slice 依赖 current EP 的写结果，则提前发射被阻止。因此，SEAM-V 的 cross-EP overlap 是受限执行重叠，而不是任意跨包乱序。

【图 3：Hybrid Execution and Safe Cross-EP Overlap】
...
首先，sequencer 利用 `ep_id` 进行 EP-aware dependency handling。传统 sequencer 需要根据在飞命令维护 RAW、WAR 和 WAW 检查；SEAM-V 在在飞向量指令状态中记录 EP 身份，并在新请求到来时识别同包候选冲突。对于由同一 EP 元数据标识、并通过本地 readiness 与 ordering 检查确认可安全忽略的候选冲突，sequencer 可将其从保守 hazard 集合中剔除；真实寄存器相关和跨 EP 顺序约束仍保持不变。该机制不移除 scoreboard，也不绕过真实数据依赖，而是在后端动态管理框架内利用前端提供的包级独立性信息减少假相关阻塞。

其次，VLSU 利用请求级访存提示与 loop-active 上下文进行 EP-driven memory anticipation。`prefetch_disable` 显式决定是否关闭预取；在未关闭时，`prefetch_mode` 选择 1X、2X、4X 或 8X 的预取距离。将访存提示绑定到向量请求，而非仅依赖瞬时 header 信号，可以避免前端推进、packet 间隙或请求排队导致的语义错位。地址生成逻辑结合预取距离、队列占用和在途请求信用进行流控，避免预取请求压制 demand load/store。这样，软件和前端已知的循环访存意图被转化为后端可执行的访存提前化行为。

最后，HEU 的 buffered vector early issue 与 VDU command window 共同支持跨 EP 执行重叠。在 scalar/control 约束满足且 memory ordering 安全的情况下，后续 EP 的部分 vector-side work 可以提前进入后端队列，不必等待前一 EP 的所有向量事件完全完成；后端则根据 `ep_id` 区分请求归属，并在 sequencer、VLSU 和响应路径中维护正确状态。该机制使标量控制、向量计算和访存访问能够在 EP 粒度上形成时空重叠。
...
前一节说明了 SEAM-V 如何在硬件中生成、传递并消费 EP 语义。本节从软件侧定义这些语义如何表达。SEAM-V 的软件接口遵循两个原则：第一，热点 kernel 仍以标准 RVV 指令表达其计算语义；第二，软件将局部并行性、控制边界和访存意图以轻量级标注形式暴露给硬件。由此，SEAM-V 在普通 RVV 编程模型之上建立显式的软件-硬件契约，使硬件能够利用程序结构，而不依赖完全动态的后端推断。

### 4.1 Task-Level Execution Model

SEAM-V 以 task 为基本执行单元。主程序负责数据准备、参数传递和 task 提交，热点 RVV kernel 被封装为 SEAM-V task 并交由硬件执行。Task 内部仍使用标准 RVV 指令描述向量计算，但其取指、标量控制、向量派发和 EP 形成由 SEAM-V 本地前端推进。相比 host 逐条投放向量指令的紧耦合模式，task-level execution 将软件接口提升到 kernel 粒度，降低 host 参与度，并为前端包级组织提供稳定执行上下文。

这一模型将软件侧职责划分为两层：算法层仍以普通 RVV kernel 表达数据并行计算；执行结构层则通过 HINT header、p-bit 和访存提示表达可供硬件利用的包级语义。换言之，SEAM-V 并不要求软件显式管理向量后端状态，而是要求软件在可确定的局部范围内描述哪些指令可被组织为 EP、哪些位置构成控制或访存边界，以及哪些规则访存流具有可提前化的规律。Task 的结束由标量后端识别的 task-end 指令触发；本文示例采用 `ret` 表示任务返回。

### 4.2 HINT Header Format and EP Annotation

SEAM-V 使用 RISC-V HINT 形式的 `lui x0, imm20` 描述 EP 形成规则。该指令不改变普通 RISC-V architectural state；在 SEAM-V 中，其立即数字段被解释为 packet 级元信息。在不支持 SEAM-V 的标准 RISC-V 实现上，该编码保持为无副作用 HINT/NOP，只是不触发 EP 形成与后端语义传播。表 1 给出当前 HINT header 格式。
...
| `prefetch_mode`    | `imm20[18:17]` | 预取距离；在 `prefetch_disable=0` 时，`00`=1X，`01`=2X，`10`=4X，`11`=8X |
...
访存提示由 `prefetch_disable` 和 `prefetch_mode` 共同定义。当 `prefetch_disable=1` 时，该 packet 显式关闭预取；当 `prefetch_disable=0` 时，`prefetch_mode` 编码预取距离，分别表示 1X、2X、4X 和 8X。该设计将“是否启用预取”和“预取距离选择”解耦，避免将 `00` 同时用作关闭语义和最小预取距离语义。
...
Listing 1 给出一个简化的 HDV 标注向量 kernel 片段，用于说明 HINT header、p-bit、`packet256`、cross-packet packing 以及 loop/prefetch annotation 的关系。该示例是说明性伪代码，用伪宏 `HDV_HINT` 表达软件可见标注语义，并不表示 bit-accurate 的 `imm20` 编码、真实 slot 分组或完整可编译 kernel；实际实现中，标注由汇编宏展开为对应的 HINT header。示例中的 `P_SPLIT` 表示 logical packet 内部仍按照真实依赖、控制边界和后端安全条件切分为多个 EP，不表示将 header 覆盖范围内的所有业务指令强制合并为同一 EP；`prefetch_mode=1X` 隐含 `prefetch_disable=0`。
...
该示例展示了三类软件意图。第一，`packet256` 允许一个 HINT header 覆盖更大的 logical packet，从而摊销 header 的静态代码空间和取指开销；logical packet 内部仍可通过 p-bit 切分为多个 EP，以保留真实依赖、控制边界和硬件安全条件。第二，`cross` 允许 logical packet 尾部 EP 在满足 p-bit 和安全条件时跨入下一 logical packet 开头继续打包，从而减少因取指包边界造成的 EP 人为截断。第三，loop 标记和 `prefetch_mode=1X` 将循环上下文与规则 unit-stride 访存意图暴露给后端。实际 kernel 可根据依赖关系、控制边界和访存模式选择更保守或更积极的 EP 标注。

### 4.4 Memory Intent and Correctness Boundary

Prefetch annotation 描述循环体内规则 unit-stride load 的访存提前化意图，主要面向同一访存流在后续迭代或后续 VL chunk 中可预测的地址访问。软件首先通过 `prefetch_disable` 显式决定是否关闭预取；在启用预取时，再通过 `prefetch_mode` 选择 1X、2X、4X 或 8X 的预取距离，使后端能够对相应 request-bound 访存流提前发起后续访问。对于 gather/scatter、不规则访存、短促执行阶段或难以由固定距离建模的访问模式，软件应关闭该提示，使后端退化为普通 demand-driven 访存执行。
...
SEAM-V 保持 RVV 软件模型不变。EP、p-bit、loop 标记、`prefetch_disable` 和 `prefetch_mode` 均为 SEAM-V 微架构解释的信息，而不是新的 RVV 指令语义。软件负责遵守 EP annotation contract，不将存在未解决依赖、控制不确定性或内存顺序风险的指令错误合并；硬件负责维护 RVV 后端的动态依赖管理、寄存器一致性和内存定序。保守标注不会影响功能正确性，只会减少可利用的包级语义。由此，SEAM-V 在不改变算法语义的前提下，为 RVV kernel 提供可被硬件消费的显式执行结构。
...
下面给一版**约 40 条**、更适合你这篇 SEAM-V/HDV 论文的参考文献。我的选择原则是：先保证正文现有 `[1]–[8]` 能支撑 §1–§2 的论证，再补足 RVV 实现、经典向量机、解耦执行、VLIW/EPIC、后端调度、scoreboard/hazard、prefetch 和你的前期工作。Ara/New Ara/Ara2 是最核心基线；Saturn 适合放在后端调度相关工作；SVE/A64FX 用来说明 VLA/scalable vector 的大背景；Jouppi、Mowry、Vanderwiel 等用于支撑预取相关讨论。Ara 系列和 Saturn 的公开摘要也明确覆盖了 RVV 处理器、scalar core issue-rate、vector scheduling 等与你正文最贴近的问题。([arXiv](https://arxiv.org/abs/1906.00478?utm_source=chatgpt.com))
...
[2] RISC-V International, “The RISC-V Instruction Set Manual, Volume I:
    Unprivileged Architecture,” latest available version.

[3] K. Asanović and D. A. Patterson, “Instruction Sets Should Be Free:
    The Case for RISC-V,” EECS Department, University of California, Berkeley,
    Technical Report UCB/EECS-2014-146, 2014.

[4] J. L. Hennessy and D. A. Patterson, Computer Architecture:
    A Quantitative Approach, 6th ed. Morgan Kaufmann, 2017.

[5] R. M. Russell, “The CRAY-1 Computer System,” Communications of the ACM,
    vol. 21, no. 1, pp. 63–72, 1978.

[6] R. Espasa, M. Valero, and J. E. Smith, “Vector Architectures:
    Past, Present and Future,” in Proc. International Conference on
    Supercomputing (ICS), 1998.

[7] N. Stephens et al., “The ARM Scalable Vector Extension,”
    arXiv:1803.06185, 2018.

[8] T. Yoshida, “Fujitsu High Performance CPU for the Post-K Computer,”
    in Proc. Hot Chips 30, 2018.

[9] M. Cavalcante, F. Schuiki, F. Zaruba, M. Schaffner, and L. Benini,
    “Ara: A 1 GHz+ Scalable and Energy-Efficient RISC-V Vector Processor
    with Multi-Precision Floating Point Support in 22 nm FD-SOI,”
    IEEE Transactions on Very Large Scale Integration (VLSI) Systems,
    vol. 28, no. 2, pp. 530–543, 2020.

[10] M. Perotti, M. Cavalcante, N. Wistoff, R. Andri, L. Cavigelli,
     and L. Benini, “A ‘New Ara’ for Vector Computing: An Open Source
     Highly Efficient RISC-V V 1.0 Vector Processor Design,”
     arXiv:2210.08882, 2022.

[11] M. Perotti, M. Cavalcante, R. Andri, L. Cavigelli, and L. Benini,
     “Ara2: Exploring Single- and Multi-Core Vector Processing with an
     Efficient RVV 1.0 Compliant Open-Source Processor,” arXiv:2311.07493, 2023.

[12] M. Perotti, S. Riedel, M. Cavalcante, and L. Benini,
     “Spatz: Clustering Compact RISC-V-Based Vector Units to Maximize
     Computing Efficiency,” arXiv:2309.10137, 2023.

[13] N. K. Purayil, M. Perotti, T. Fischer, and L. Benini,
     “AraXL: A Physically Scalable, Ultra-Wide RISC-V Vector Processor
     Design for Fast and Efficient Computation on Long Vectors,”
     arXiv:2501.10301, 2025.

[14] J. Zhao, D. Grubb, M. Rusch, T. Wei, K. Anderson, B. Nikolić,
     and K. Asanović, “Instruction Scheduling in the Saturn Vector Unit,”
     arXiv:2412.00997, 2024.

[15] J. K. L. Lee, M. Jamieson, N. Brown, and R. Jesus,
     “Test-driving RISC-V Vector Hardware for HPC,” arXiv:2304.10319, 2023.

[16] M. H. AskariHemmat et al., “Quark: An Integer RISC-V Vector Processor
     for Sub-Byte Quantized DNN Inference,” arXiv:2302.05996, 2023.

[17] T. Dupuis et al., “Sparq: A Custom RISC-V Vector Processor for Efficient
     Sub-Byte Quantized Inference,” arXiv:2306.09905, 2023.

[18] C. Wang, C. Fang, X. Wu, Z. Wang, and J. Lin,
     “SPEED: A Scalable RISC-V Vector Processor Enabling Efficient
     Multi-Precision DNN Inference,” arXiv:2409.14017, 2024.

[19] F. Zaruba and L. Benini, “The Cost of Application-Class Processing:
     Energy and Performance Analysis of a Linux-ready 1.7GHz 64bit
     RISC-V Core in 22nm FDSOI Technology,” arXiv:1904.05442, 2019.

[20] J. E. Smith, “Decoupled Access/Execute Computer Architectures,”
     in Proc. International Symposium on Computer Architecture (ISCA),
     pp. 112–119, 1982.

[21] A. Weber, K.-A. Tran, S. Kaxiras, and A. Jimborean,
     “Decoupled Access-Execute on ARM big.LITTLE,” arXiv:1701.05478, 2017.

[22] R. Szafarczyk, S. W. Nabi, and W. Vanderbauwhede,
     “Compiler Support for Speculation in Decoupled Access/Execute
     Architectures,” arXiv:2501.13553, 2025.

[23] J. A. Fisher, “Very Long Instruction Word Architectures and the ELI-512,”
     in Proc. International Symposium on Computer Architecture (ISCA),
     pp. 140–150, 1983.

[24] J. A. Fisher, “Trace Scheduling: A Technique for Global Microcode
     Compaction,” IEEE Transactions on Computers, vol. C-30, no. 7,
     pp. 478–490, 1981.

[25] B. R. Rau and J. A. Fisher, “Instruction-Level Parallel Processing:
     History, Overview, and Perspective,” Journal of Supercomputing,
     vol. 7, no. 1–2, pp. 9–50, 1993.

[26] M. S. Schlansker and B. R. Rau,
     “EPIC: Explicitly Parallel Instruction Computing,” Computer,
     vol. 33, no. 2, pp. 37–45, 2000.

[27] J. C. Dehnert, P. Y.-T. Hsu, and J. P. Bratt,
     “Overlapped Loop Support in the Cydra 5,” in Proc. ASPLOS,
     pp. 26–38, 1989.

[28] B. R. Rau, “Iterative Modulo Scheduling: An Algorithm for Software
     Pipelining Loops,” in Proc. International Symposium on Microarchitecture
     (MICRO), pp. 63–74, 1994.

[29] Texas Instruments, TMS320C6000 CPU and Instruction Set Reference Guide,
     Texas Instruments, latest available revision.

[30] J. E. Thornton, Design of a Computer: The Control Data 6600,
     Scott, Foresman and Company, 1970.

[31] R. M. Tomasulo, “An Efficient Algorithm for Exploiting Multiple Arithmetic
     Units,” IBM Journal of Research and Development, vol. 11, no. 1,
     pp. 25–33, 1967.

[32] N. P. Jouppi, “Improving Direct-Mapped Cache Performance by the Addition
     of a Small Fully-Associative Cache and Prefetch Buffers,”
     in Proc. International Symposium on Computer Architecture (ISCA),
     pp. 364–373, 1990.

[33] T. C. Mowry, M. S. Lam, and A. Gupta,
     “Design and Evaluation of a Compiler Algorithm for Prefetching,”
     in Proc. ASPLOS, pp. 62–73, 1992.

[34] S. P. VanderWiel and D. J. Lilja,
     “Data Prefetch Mechanisms,” ACM Computing Surveys,
     vol. 32, no. 2, pp. 174–199, 2000.

[35] K. J. Nesbit and J. E. Smith,
     “Data Cache Prefetching Using a Global History Buffer,”
     in Proc. International Symposium on High-Performance Computer
     Architecture (HPCA), pp. 96–107, 2004.

[36] J. Lee, H. Kim, and R. Vuduc,
     “When Prefetching Works, When It Doesn’t, and Why,”
     ACM Transactions on Architecture and Code Optimization,
     vol. 9, no. 1, 2012.

[37] Intel Corporation, Intel 64 and IA-32 Architectures Optimization
     Reference Manual, latest available revision.

[38] A. Seznec, “A Case for Two-Way Skewed-Associative Caches,”
     in Proc. International Symposium on Computer Architecture (ISCA),
     pp. 169–178, 1993.

[39] M. E. Wolf and M. S. Lam, “A Data Locality Optimizing Algorithm,”
     in Proc. ACM SIGPLAN Conference on Programming Language Design
     and Implementation (PLDI), pp. 30–44, 1991.

[40] <你的前期工作>, “Boosting Vector Instruction Throughput in RISC-V
     via a Hybrid Decoupled Architecture with VLIW-Driven Execution,”
     <venue/status>, <year>.
```

## 为什么这样选

**[1]–[4]** 是基础层：RVV 规范、RISC-V ISA 背景、RISC-V 开放 ISA 动机和体系结构通用教材。它们支撑引言和 §2.1 中关于 RVV、向量长度无关、ISA 兼容性和基本体系结构概念的论述。

**[5]–[8]** 是经典/可伸缩向量背景。[5] CRAY-1 支撑经典向量机和 chaining 传统；[6] 是向量体系结构综述；[7] SVE 是 RVV 之外最重要的 vector-length-agnostic 对照；[8] A64FX/SVE 说明可伸缩向量 ISA 在高性能处理器中的实际落地。SVE 论文明确强调 vector-length agnostic 编程模型和实现可伸缩性；A64FX 是 SVE 在 Fugaku 上的重要硬件实例。([arXiv](https://arxiv.org/abs/1803.06185?utm_source=chatgpt.com))

**[9]–[15]** 是 RVV/向量处理器核心相关工作。Ara 是你的直接基线；New Ara 与 Ara2 贴近 RVV 1.0、开源实现和 scalar issue-rate/short-vector bottleneck；Spatz/AraXL 展示 RVV 处理器在小型集群和超宽长向量方向的扩展；Saturn 是后端 vector scheduling 的近期相关工作；Test-driving RVV hardware 则补足 RVV 硬件/软件生态背景。Ara2 摘要中明确提到 scalar core、memories 和 vector architecture 是性能驱动因素，并指出多核小 lane 配置可缓解 short-vector scalar issue-rate bound；这与你的紧耦合供给瓶颈论证高度匹配。([arXiv](https://arxiv.org/abs/2311.07493?utm_source=chatgpt.com))

**[16]–[18]** 是 DNN/低精度 RVV 扩展方向，放在引言或评估背景中可支撑“DNN 推理、多精度/低精度、edge workload”场景，但不建议在相关工作主线中过度展开。它们说明 RVV 处理器正在被用于专用 DNN 推理，但 SEAM-V 的贡献不是新低精度算子，而是执行组织和后端可见 EP。Quark/Sparq/SPEED 都是 RVV-oriented DNN acceleration 的代表。([arXiv](https://arxiv.org/abs/2302.05996?utm_source=chatgpt.com))

**[19]** 是 CVA6/Ariane 标量核背景。因为 SEAM-V 讨论“标量核逐条驱动向量后端”的代价，引用 Ariane/CVA6 的能效分析比只引用 Ara 更完整。

**[20]–[22]** 是解耦执行。Smith 1982 是 decoupled access/execute 的根文献；[21]、[22] 作为现代 DAE 示例说明解耦仍需要处理控制依赖、推测和访问/执行片段之间的语义保持问题。DAE 的思想与 SEAM-V 的“解耦供给”有关，但 SEAM-V 的区别是把 EP 语义显式传到 RVV 后端。

**[23]–[29]** 是 VLIW/EPIC/packetization 和软件流水背景。Fisher、Rau、Schlansker 是必须保留的经典引用；Cydra-5、modulo scheduling 和 TI DSP 可作为 packetized execution、software pipelining、DSP/VLIW 的补充。VLIW 和 EPIC 的核心思想是显式暴露 ILP、降低硬件动态调度压力；你的 §2.3 正好需要说明为什么这类静态执行包不能直接替代 RVV 后端动态管理。([维基百科](https://en.wikipedia.org/wiki/Very_long_instruction_word?utm_source=chatgpt.com))

**[30]–[31]** 是后端 hazard/dynamic scheduling 的经典根文献。CDC 6600 scoreboard 和 Tomasulo 算法可以支撑 sequencer、scoreboard、RAW/WAR/WAW、动态正确性维护这条背景线。你不需要在正文里长讲历史，但它们适合放在 §2.1/§2.3 的后端动态管理背景中。([维基百科](https://en.wikipedia.org/wiki/James_E._Thornton?utm_source=chatgpt.com))

**[32]–[37]** 是 prefetch 相关工作。Jouppi 的 prefetch buffers 是硬件预取经典起点；Mowry 是编译器预取；VanderWiel/Lilja 是综述；Nesbit/Smith 是相关性/历史型预取；Lee/Kim/Vuduc 说明 prefetch 并非总有效；Intel 手册用于工程背景。它们共同支撑你后文区分“普通 runtime prefetch”与“EP-driven/request-bound prefetch”。预取的一般目标是提前取回未来可能需要的数据或指令，但错误预取也会浪费带宽或造成污染，这与你 §4.4 的“prefetch 是性能提示而非正确性条件”一致。([维基百科](https://en.wikipedia.org/wiki/Prefetching?utm_source=chatgpt.com))

**[38]–[39]** 是缓存/数据局部性补充文献。它们不是 SEAM-V 的核心，但可以支撑访存行为、局部性优化、cache/prefetch 背景，避免 VLSU/prefetch 讨论显得孤立。

**[40]** 是必须保留的前期工作。正文 §2.4 需要明确：前期工作提供前端解耦与 EP/VLIW 打包基础，本文增量是后端可见 EP、EP-driven backend cooperation 和完整系统/PPA 评估。这个引用最好放在 Positioning of SEAM-V 中，而不是只放在实验或脚注里。
