...
`hardware/src/scala_backend/hdv_scalar_backend.sv`，分析同一个 HDV
execute packet（EP）内可能暴露给标量后端的计算需求，并据此讨论当前标量后端的实现选择。

> 当前状态说明：本文最初用于评估“单发射标量后端是否太窄”。当前 RTL 已推进到 `ScalarIssueWidth=3`、`SimpleAluIssueWidth=2`：一个 EP 内最多两条 simple ALU 指令加一条 complex 指令同周期发射。因此文中关于“单发射是当前实现”的历史分析，只作为旧基线；当前实现以本说明和第 5/8/11 节更新后的描述为准。
...
| `win3_all32_scalar` | 假设普通 128-bit beat 中业务指令大多为 32-bit，任意 3 条连续业务指令窗口内的标量指令数量 | 贴近一个 128-bit beat 中 header 后的 3 条 32-bit 业务指令 |

当前 VLIWPU 的 packet/EP 结构是：

- 外部 fetch beat 为 128 bit；
- 低地址第一个 32-bit word 是 `lui x0, imm20` HDV hint header；
- header 后 96 bit 是 6 个 16-bit slot；
- header `packet256=1` 时可把两个 128-bit beat 组成 256-bit logical packet；
- 32-bit 指令占 2 个 slot；
- 当前 `NumSlots=8`，所以一个 EP 最多容纳 8 个 halfword slot；
- 如果 EP 中全是 32-bit 指令，一个 EP 最多容纳 4 条业务指令；普通 128-bit beat 内最多 3 条，跨包 carry 或 256-bit logical packet 可达到 4 条；
...
| EP2 | `sub a0,a0,t0` + `vle32.v` + `slli t1,t0,2` | 2 integer ALU | 当前标量后端两条 simple ALU lane 可同周期覆盖 |
| EP3 | `vfmacc.vf` + `add a1,a1,t1` + `vse32.v` | 1 integer ALU | vector 和 scalar 可并行启动，EP acknowledged 受较慢一侧限制 |
| EP4 | `add a2,a2,t1` + `bnez a0,loop` | 1 integer ALU + 1 branch | simple ALU 与 complex branch lane 可同周期处理，branch 产生 redirect |

这个真实程序说明：即使是最简单的 vector loop，标量侧也会出现同 EP 多条简单 ALU/branch 的需求。当前 2 simple ALU + 1 complex lane 能覆盖 `sub+slli`、`add+branch` 这类组合；如果同 EP 出现 3 条以上 simple ALU，仍会多周期处理。
...
| `vsgemm.row_block_loop_128` | 16 | 6 | 6 ALU | 3 | 多个地址更新，单发射会拉长 EP acknowledged |
...
| 项目 | 当前 HDV 行为 |
|---|---|
| Fetch packet | 外部 128-bit beat，其中低地址第一个 32 bit 为 `lui x0, imm20` HDV hint header，后 96 bit 为 6 个 16-bit slot |
| p-bit 来源 | `lui` 的 `imm20[12:0]`，同时有 `packet256/cross/loop_start/loop_end` 控制位 |
| EP 最大容量 | 当前 `NumSlots=8`，最多 8 个 halfword slot；全 32-bit 指令时最多 4 条业务指令 |
...
| 同 EP 执行 | HEU 把 EP 分成 scalar slice/vector slice；vector 发 Ara，scalar 发 `hdv_scalar_backend` |
| Scalar slice | 当前后端为轻量 3 发射：2 simple ALU + 1 complex lane，hazard/order barrier 时保守拆开 |
...
| Loop lock | 当前偏向自动检测/缓存 fetch packet，header 有显式 loop_start/loop_end 标记，但尚未等价于 C66x SPLOOP 的完整软件流水 loop buffer 语义 |
...
- scalar 后端当前是轻量 3 发射 CVA6-style backend；
...
C66x 一个 EP 最多 8 条指令；当前 HDV 一个 EP 最多 8 个 halfword slot。若全是 32-bit RISC-V 指令，最多容纳 4 条业务指令；普通 128-bit beat 内只有 3 条业务指令，需要显式 cross 或 packet256 才可能达到 4 条。
...
- scalar backend 当前只兑现 2 simple ALU + 1 complex 的轻量并行，还没有 C66x 那种完整 functional-unit 绑定和资源模型。
...
当前 `hdv_scalar_backend` 的关键行为：

- 一次从 HEU 接收一个 EP 的 scalar slice；
- `scalar_ready_o = (state_q == IDLE)`，后端忙时不能接收新的 scalar slice；
- `scalar_ep_done_o = (state_q == DONE)`，表示当前 scalar slice 中所有 scalar slot 都已处理完；
...
- 直到 scalar slice 清空后才 done。

这是一种功能优先、面积较小、控制简单的设计。它和当前 HEU 的保序模型匹配：HEU 只有在 scalar slice done、vector slice acknowledged 后，才认为 EP acknowledged，之后再推进后续 EP 或处理 redirect。
...
结论：**当前 2 simple ALU + 1 complex lane 是合理起点，比单发射明显更贴近 HDV/VLIW 的 EP 语义；继续加宽前应先用 EP 统计证明收益。**

原因如下：

1. `vsaxpy_hdv` 已经有 2 条标量操作落在同一个 EP 的情况。
2. 大多数 hand-written vector kernel 的局部窗口中，3 条业务指令内经常有 2 到 3 条标量指令。
3. 当前 p-bit 的语义本来就是表达并行关系；2 simple ALU lane 能利用 EP 内常见的双地址/计数更新。
4. complex lane 覆盖 branch/LSU/FPU/MUL/CSR 中的一条，适合常见 loop 尾部。
5. 当前 EP 容量为 8 个 halfword slot，全 32-bit 场景最多 4 条业务指令；直接做更宽完整标量通路是否划算，需要更精确的 EP 分布数据。

所以更科学的判断是：

- 1-wide：实现简单，适合 bring-up，但会成为明显瓶颈；
- 当前 2 simple ALU + 1 complex：性价比最高，能覆盖 `vsaxpy_hdv`、copy/swap/dot/dwt/gemm loop 中最常见的组合；
- 3-wide full scalar 或更多 complex lane：只有在大量 scalar LSU/FP/MUL 同 EP 出现，或更宽 EP/256-bit logical packet 稳定后才更有必要；
- 4-wide 或更宽：当前 EP 容量下容易面积/时序投入大于收益。

当前跨包 carry 已改为 header 显式 `cross=1` 控制。显式跨包能让不同 packet 的尾部/头部进入同一 EP，使 3 发射收益更稳定；但 EP 仍受 `NumSlots=8` 限制，全 32-bit 指令最多 4 条业务指令。

## 9. 当前后端结构与后续扩展

当前 RTL 已采用“可并行的简单通路 + 保守仲裁的复杂通路”：

- 2 条 simple ALU lane：覆盖 LUI/AUIPC/OP-IMM/OP-IMM-32/OP/OP-32。
- 1 条 complex lane：覆盖 branch/jal/jalr/CSR/MULT/FPU/LSU，以及不能进入 simple lane 的指令。
- EP 内做轻量 hazard 检查，不引入完整 CVA6 scoreboard/commit。
...
| 资源 | 当前/建议 |
|---|---|
| Integer ALU | 2 simple lanes |
| Branch | 1 complex lane，可与 simple ALU 同周期执行 |
...
|---|---|---|
| EP 内并行 | 当前支持 2 simple ALU + 1 complex lane | 继续扩展到更多 lane 或更细功能单元绑定 |
| 资源检查 | 后端有轻量 lane/hazard 检查 | VLIWPU/后端需要更显式检查 ALU/BR/LSU/FPU/MUL 资源 |
| 寄存器文件 | 已支持当前 3 发射所需的读写/仲裁模型 | 更宽发射需要更多读口/写口或 bank/replica/仲裁 |
| 写回冲突 | simple batch 检查重复 rd，complex 与 simple 做冲突检查 | 同 EP 多写回必须由 packer/后端共同禁止或定义优先级 |
| branch | 串行解析 | 一个 EP 内最多一个 branch，branch lane 独立解析 |
| scalar LSU | complex lane 内单 outstanding AXI FSM | 至少允许 simple ALU 与 LSU request 重叠，后续可考虑小 load buffer |
| done 语义 | scalar slice 全部完成后 done | 保持这个语义，但内部用 per-lane done 汇聚 |
...
- `scalar_ep_done_o` 表达“对后续 EP 安全完成”，语义保守正确；
- 对 bring-up、分支 redirect、vset 写回 hazard、vector scalar operand service 都比较友好；
- 面积和控制复杂度低。

当前实现的不足：

- EP 内 scalar 并行只覆盖 2 simple ALU + 1 complex lane，还不能表达更细的 VLIW functional units；
- `scalar_ready_o` 只在 IDLE 拉高，不能缓存下一组 scalar slice；
- scalar LSU/FPU/MUL 仍在 complex lane 内仲裁，多条复杂指令不能同周期；
- 如果未来把更多 Ara benchmark 转成 HDV hint 程序，complex lane、scalar slice buffering 和更明确的资源绑定会成为后续瓶颈。
...
这个阶段不追求完整 CVA6 级性能，但已经利用 EP 内最常见的简单标量并行，避免单发射成为显著 bring-up 瓶颈。
...
- scalar slice done 仍等待所有 lane 完成；
...
4. scalar_ep_done_o 表示所有 lane 都完成，还是只表示指令已进入 lane queue？
...
- `scalar_ep_done_o` 仍建议表示“该 scalar slice 对后续 EP 已安全完成”，否则 HEU 的依赖语义要整体重写。
