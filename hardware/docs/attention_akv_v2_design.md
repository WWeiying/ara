...
and models FULL/REFILL context replacement. Invalid selectors preserve a
committed context. A payload memory fault is a different class in RTL:
partially written hidden storage is never made ready, and the failed context
is invalidated.
...
Qwen3 introduces a distinct admitted shape, D128/GQA2. The context and SRAM
organization already support two Query rows, so no additional resident state
is required. Its native QK kernel preserves the original F32 Query, replays
resident F16 K in four-column panels, widens K to F32, and accumulates both
Query rows with the existing RVV FP32 resources. On a GQA2 subgroup derived
without regenerating values from the real Prefill capture (`M=15`, active
KV=15), AKV-v2 passes with zero mismatches in 189,123 cycles versus 268,743
cycles for the matched strong tiled-RVV path: 1.421x speedup and 29.63% fewer
cycles. Strict activity is one FULL, 480 panel commands, 1,920 logical K
columns, 240 V-row loads, zero bank conflicts, and zero rejected commands.
This establishes native D128/GQA2 arithmetic and context reuse; full-model
QEMU remains functional rather than cycle-accurate evidence.

This operator result does not by itself admit every short Prefill graph. GGML
uses its sequential `one_chunk` Attention implementation when the Query length
is below `GGML_FA_TILE_Q=64`, then changes to tiled Attention at 64. The two
paths have different numerical schedules: `one_chunk` converts Query to the K
dot-product type and updates an F16 output numerator one visible KV token at a
time, whereas tiled Attention keeps F32 Query/output state and performs
tile-level reductions. AKV Prefill follows the latter contract. A real Qwen3
M33 stage probe showed identical Query input but the first two QK scores
differed by one and three FP32 ULPs (`0x41180371/0x41180370` and
`0x40ef2e82/0x40ef2e7f`); the difference then propagated through Softmax and
PV. The same M33 AKV operator took 406,205 cycles versus 320,275 for the
independent standard-RVV Q64/KV64 tiled baseline. Thus the candidate neither
matches GGML's short-query numerical contract nor beats the strong measured
RVV denominator. The production selector therefore leaves M<64 on `one_chunk`
rather than treating numerical drift as an error to tune away.

At M84, where GGML itself selects tiled Attention, the real Qwen3 D128/GQA2
graph executes AKV Prefill with 57,120 causal attention pairs and 14,622,720
MACs. A raw-word comparison of all 172,032 F32 outputs at the selected
Attention node is bit-exact between QBS-only and QBS+AKV-v2. An M33 rerun after
the selector change is likewise bit-exact because it correctly remains on the
ordinary path. These two boundary checks establish that the selector follows
the numerical algorithm boundary rather than an arbitrary minimum-work rule.

For numerical alignment with the ordinary GGML/RVV Prefill path, each tile's
F32 exponential terms are reduced through standard RVV widening accumulation
into F64. The online denominator state remains F32 between tiles: the old state
is first rescaled and rounded as F32, the F64 tile sum is added, and the new
state is rounded back to F32. This is a software schedule over the existing
RVV FP resources, not a new AKV arithmetic datapath.
...
Decode plus tiled Prefill with at least `GGML_FA_TILE_Q=64` Query tokens, F32
Query, F16 K/V and mask, F32 output, one batch, `D64/D96/D128`, GQA ratios
`1..8`, the supported stride/layout contract, and no sinks, ALiBi, or softcap.
KV is tiled in groups of at most 64 tokens, including arbitrary final tails.
It selects the shared version-2 planner and native kernel only when capability
discovery and every condition agree. Short Prefill and any unsupported shape
or feature return before hidden context state changes and execute the existing
...
1% gate. The `D128/GQA6` arithmetic path is fully specialized assembly.
D128/GQA2 has a specialized F32 panel4 QK kernel and reuses the generic F32 PV
kernel; other admitted shapes use generic RVV arithmetic over the same v2
context. The matrix therefore proves correctness and selection breadth rather
than uniform speedup. The generalized Host census passes 28/28 real model/KV cases
