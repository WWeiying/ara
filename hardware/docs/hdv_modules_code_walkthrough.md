...
- **packet 末尾**：到达 fetch packet 最后一个 slot 时，普通情况下结束；如果该尾部 EP 不是 SYSTEM/BRANCH、没有用满 issue 宽度，则可以进入跨包 carry，和下一 fetch packet 开头继续组成同一个 EP。

`MaxIssueSlots=NumSlots`（即 6），等效于每个 EP 最多 6 个 16-bit slot。跨包 EP 仍受这个宽度限制：例如 carry tail 已占 2 个 slot，则最多还能从下一 fetch packet 开头并入 4 个 slot。
...
这样，一个 fetch packet 可以拆分成多个 execute packet（由 p-bits 和边界条件决定），也可以在所有 p-bits=1、无 SYSTEM/BRANCH 的情况下整包形成一个 execute packet。若最后一个 EP 符合跨包条件，packet 末尾不会立即发出，而是先进入 carry。
...
| EP2 | packet 0 tail + packet 1 head | sub + vle32.v + slli | packet 0 尾部 `sub` 进入 carry，和 packet 1 开头压缩成跨包 EP |
| EP3 | packet 1 tail + packet 2 head | vfmacc.vf + add + vse32.v | packet 1 尾部 `vfmacc` 进入 carry，和 packet 2 开头压缩成跨包 EP |
| EP4 | packet 2 tail + packet 3 head | add + bnez | packet 2 尾部 `add` 进入 carry，遇到 packet 3 的 branch 后停止 |

taken iteration 合计 **4 个 execute packet**。最后一轮 branch not-taken 后，还会继续接受 packet 3 剩余的 `ret` 和 `nop` 占位 EP，因此总数是 `(iters-1)*4 + 6`。

### packet_ready 逻辑

```systemverilog
assign current_packet_drained = packet_hold_valid_q
...
VLIWPU 没有保存 packet 时可以接收新 packet；如果当前 packet 的最后一个 EP 正在被 HEU 接收，也同周期向 IPU 拉 ready。若当前尾部 EP 符合跨包条件，`tail_cross_candidate` 会提前向 IPU 拉 ready，让下一 packet 尽快进入 VLIWPU。carry 已经有效但还没有下一 packet 时，只在 `packet_hold_valid_q=0` 时接收下一包。
...
当前 HEU 默认仍把 EP 作为前端依赖推进边界：buffer EP 可以先进入 skid buffer，但它的 scalar/vector slice 要等 promote 为 current EP 后再发给后端。代码保留了 `EnableBufferedVectorEarlyIssue` 参数，默认值为 `1'b0`，因此 buffered EP 的 vector slice 默认不会跨 current EP 提前发射。

保留这组逻辑的原因是以后仍可能重新评估该优化，但当前实测显示 HEU 侧抢发没有解除真实标量模式的主要等待：vector dispatch 必须先捕获每条 vector 指令的 scalar operand，普通 vector EP 才能 accepted；`vset rd!=x0` 还要等 response 写回。
...
这个 table 是真实标量模式 accepted 的关键配套逻辑：vector dispatch 可能因为 request drain 或 `vset` response 写回而延迟 accepted，因此需要用 `ep_id` 标记等待项。默认关闭 HEU buffered vector 提前发射时，它仍用于区分可能排队的 vector EP；如果以后重新打开提前发射，也不会因为没有 tag 而清错 HEU pending。`vec_heu_accepted_o` 在真实标量模式下由 registered wait-table 状态组合产生，避免额外打一拍；`vec_heu_ready_o` 还会把本周期 ready 可弹出的 table entry 视为空位，从而支持同周期 accepted 一个旧 EP 并接收一个新 EP。
...
| `carry_slot_valid_q` | [5:0] | VLIWPU 内部 | carry tail 中哪些压缩 slot 有效 | 典型为 6'b000011 |
| `carry_pc_q` | [63:0] | VLIWPU 内部 | carry tail 第一条指令 PC | 跨包 EP 的 base PC |
| `vliwpu_heu_execute_slot_pc_o` | [5:0][63:0] | VLIWPU → HEU | 每个输出 slot 的真实 PC | 跨包 EP 中 branch 仍保持原始 PC |
...
| `vliwpu_heu_execute_slot_valid_o` | [5:0] | VLIWPU → HEU | EP slot 掩码 | 普通 EP 同 issue_mask；跨包 EP 为压缩后的 slot0..N |
...
| EP2 | packet 0 tail + packet 1 head | 6'b111111 | sub + vle32.v + slli |
| EP3 | packet 1 tail + packet 2 head | 6'b111111 | vfmacc.vf + add + vse32.v |
| EP4 | packet 2 tail + packet 3 head | 6'b001111 | add + bnez |

最后一轮 not-taken 后，还会继续看到 `ret` 和 `nop` 两个占位 EP。1024 elements、VL=32 时 expected EP 数为 `(32-1)*4+6=130`。
