...
> **近期架构更新** (2026-06):
> - **VLSU Next-VL Prefetch**: `hardware/src/vlsu/` 借鉴 ideal_execution 分支的解耦访存前端。addrgen 自动检测 unit-stride load 并提前发射预取 AR（`AXI_ID_PREFETCH=4'd1`），vldu 内 64×256bit SRAM buffer（`PrefetchQueueDepth=64`；头寄存器+1字 lookahead 隐藏 SRAM 1拍读延迟，持续消费零气泡）缓存预取返回数据，命中时旁路 demand AXI beat。幅度 `prefetch_mode` 可配 off/1X/2X/4X（匹配迭代步长 S/B）；**支持 LMUL 1/2/4/8**，addrgen 用基于 buffer 占用的信用流控（`占用×2+在途+burst≤128`）防溢出/死锁。
...
| `hdv_scalar_backend.sv` | `hdv_scalar_backend` | HDV 专用真实标量后端，复用 CVA6 部件执行 scalar slice |
| `hdv_mock_host_core.sv` | `hdv_mock_host_core` | 仿真用主核/标量后端模型，负责写 task CSR、模拟分支、计数 EP acknowledged |
...
| `vlsu/addrgen.sv` | `addrgen` | 解耦地址生成 + 预取（`prefetch_mode` off/1X/2X/4X）+ 信用流控（`prefetch_inflight_beats`）+ 页跨越段标志 ROB，`AXI_ID_PREFETCH` AR |
| `vlsu/vldu.sv` | `vldu` | 预取数据 64×256bit SRAM buffer（`PrefetchQueueDepth=64`，头寄存器+lookahead），命中旁路；支持 LMUL 1/2/4/8 |
...
BufferBytes      = 512   // current hdv_top override; module standalone default is 64
...
IPU 置 `auto_loop_lock_q`，并结合 `loop_build_q/loop_locked_q/loop_protect_q` 停止对 protected buffer 的无用背景取指，循环体常驻。`loop_start`/`loop_end` header 会保护 loop body 所在 buffer；收到 `loop_end` packet 后还会停止继续填充 512B buffer 的剩余部分，避免投机读取 kernel 后的数据区并和 Ara 数据访存抢带宽。
...
HEU 当前保留一个 current execute packet 和一个 skid buffer execute packet。只要 skid buffer 为空，就可以继续从 VLIWPU 接收下一个 EP；如果 current EP 还没达到后端 `done/acknowledged`，新来的 EP 会进入 buffer。
...
- `buffer_vector_slice_outstanding_q`: buffered vector 切片已经送出，但还没收到同 id 的 `vector_ep_acknowledged_i`。
...
- `heu_vector_ep_id_o` 给每个 vector slice 分配 1-bit EP id；`vec_ep_acknowledged_id_o` 返回同一个 id。因此 current vector pending 和 buffered vector pending 可以同时存在，HEU 按 id 清对应 pending。
...
- buffered vector 若已提前 acknowledged，等该 EP promote 为 current 时不会重复发；若还没 acknowledged，则 promote 后继续等待这次提前发射对应的 acknowledged。

`heu_scalar_valid_o` 和 `heu_vector_valid_o` 分别由寄存器输出。后端 ready 后对应 dispatch valid 清零。

### pending 和 EP acknowledged

`scalar_slice_outstanding_q`、`vector_slice_outstanding_q` 表示已经向对应后端发出 dispatch，但该后端还没有报告该切片达到前端可推进点。它们由 `scalar_ep_done_i/vector_ep_acknowledged_i` 清除。
...
`heu_top_ep_acknowledged_o` 表示一个 execute packet 已经达到 HDV 前端推进条件，不表示整个 task 完成，也不表示 Ara 内部向量指令已经退休。acknowledged 条件同时检查 dispatch valid，可以避免后端 done/acknowledged 信号异常提前时，HEU 接收下一包并覆盖尚未被 ready 握手的输出寄存器。默认配置下 buffered EP 要等 promote 为 current 后才发射；如果打开 buffered vector 提前发射，HEU 还会等相关 vector ready/acknowledged 状态归属清楚后再 promote，避免清掉仍在握手中的 buffered vector 状态。
...
- `vec_ep_ready_o` 在 pending skid buffer 为空时为 1，表示可以接收新的 vector EP。
...
- `heu_vec_prefetch_mode_i` 是随 EP 一起锁存的 prefetch mode，避免跨包/提前推进时采到错误的全局 header 值。
- `vec_ep_acknowledged_o` 是给 HEU 的一拍脉冲，表示这个 vector EP 切片已经被本模块安全接管。vtrace 模式下普通 EP 入队即可 acknowledged；真实标量模式下，要等本 EP 的 vector request 都已经被 dispatch FSM 消费，并且 scalar operand 已经被捕获到 Ara request 或 command window。带 `vset rd!=x0` 的 EP 还要等对应 Ara response 写回 scalar backend 后 acknowledged。
- `vec_ep_acknowledged_id_o` 和 acknowledged 脉冲同拍有效，返回被 acknowledged 的 vector EP id。HEU 用它分别清 current vector pending 或 buffered vector pending。
- `vec_ep_error_o` 表示 vtrace 缺失、vtrace 指令不匹配或 Ara response 报 exception。

关键点：`vec_ep_acknowledged_o` 不表示 Ara 已经真正执行完，也不表示 load/store 已经完成，更不表示向量写回已经退休。向量指令之间的数据依赖由 Ara 后端自己处理，HDV 不因为普通 vector response 等待退休。HDV 只对 scalar 可见事件做额外处理：vset granted VL、`vmv.x.s`/`vfmv.f.s` 一类 vector-to-scalar 写回，以及 `vec_store_inflight_o` 对标量内存操作的保守定序。
...
- 真实标量模式：`UseVTraceScalar=0`，通过 `vec_scalar_operand_req_valid_o` 和 `vec_scalar_rs*_addr_o` 向 `hdv_scalar_backend` 的 operand service 读取 XRF/FRF。
...
当前代码在 FSM request 和 Ara `req_ready` 之间放了一个参数化小型 command window。顶层默认 `VectorCmdWindowDepth=8`，实例化时传给本模块 `CmdWindowDepth`。它保存的是“已经选中 slot、已经拿到 scalar operand”的单条 vector request：
...
vtrace 模式下，普通 vector EP 在 `enqueue_ep && !input_ep_has_vset_wb` 时直接产生 `vec_ep_acknowledged_o`，表示本模块已经接管该 EP。因为操作数来自离线 trace，后续标量寄存器变化不会影响这些向量 request。

真实标量模式下，普通 vector EP 等所有 slot 都被 dispatch FSM 消费后才产生 `vec_ep_acknowledged_o`。这些 slot 可能已经被 Ara 接收，也可能在 command window 中等待 Ara ready；无论哪种情况，每条向量指令的标量操作数都已经从真实标量后端读出并保存。

如果 EP 中存在 `vsetvli/vsetivli/vsetvl` 且 `rd!=x0`，模块先登记该 EP 需要等待 vset 写回，暂不 acknowledged。每条发给 Ara 的 vector request 会在 `resp_meta_*` FIFO 中记录 `{wb_valid, is_fpr, is_vset, is_store, rd, ep_id}`；Ara response 返回时弹出 FIFO。如果弹出的元数据表示 scalar-visible writeback，则产生 `vec_scalar_wb_valid_o`，并用 `vec_scalar_wb_is_fpr_o` 区分写 XRF 还是 FRF，用 `vec_scalar_wb_is_vset_o` 区分是否为 vset granted VL。若是 `vset rd!=0`，还会对等待中的 vset EP 产生 `vec_ep_acknowledged_o`。

真实标量模式下，acknowledged 由两项 `real_wait_*` table 保存：
...
每个新接收的 vector EP 占一项 table。该 EP 的所有 vector slot 都被 dispatch FSM 消费后，置 `real_ep_operands_captured`；如果这个 EP 需要 `vset rd!=0` 写回，则等 response 元数据匹配该 `ep_id` 后置 `real_ep_vset_wb_done`。当 `operands_captured && (!has_vset || vset_wb_done)` 成立，模块产生 `vec_ep_acknowledged_o`，并在 `vec_ep_acknowledged_id_o` 返回该项的 `ep_id`。

这个 table 是真实标量模式 acknowledged 的关键配套逻辑：vector dispatch 可能因为 request drain 或 `vset` response 写回而延迟 acknowledged，因此需要用 `ep_id` 标记等待项。当前 HEU buffered vector 提前发射已打开，wait table 用于区分 current/buffered 两类 vector slice，避免把 acknowledged 归错 EP。`vec_ep_acknowledged_o` 在真实标量模式下由 registered wait-table 状态组合产生，避免额外打一拍；`vec_ep_ready_o` 还会把本周期 ready 可弹出的 table entry 视为空位，从而支持同周期 acknowledged 一个旧 EP 并接收一个新 EP。
...
`hdv_top` 不再扫描 dispatch packet 里的 branch opcode，也不再用 “branch done 后一拍没有 redirect” 推断 loop exit。当前 loop 控制完全由 `hdv_scalar_backend` 的精确分支解析事件驱动：
...
not-taken backward branch 直接形成 `auto_loop_exit`，送给 IPU 释放 loop wait/lock。taken branch 形成 `scalar_fast_redirect_valid`，用 `scalar_branch_target` 作为 redirect PC；同时 `scalar_backend_redirect_valid` 仍保留作为 backend redirect 路径。这个改法删除了旧的 `loop_branch_inflight_q` / `loop_branch_exit_pending_q` 推断状态，也避免 taken redirect 和 not-taken exit 因时序偏斜误配对。
...
真实模式下，`hdv_top` 内部的 `hdv_scalar_backend` 消费 HEU scalar dispatch；这些外部 `hdv_scalar_*` 信号仍然导出，主要用于 testbench/调试观察。关闭真实标量后端时，外部 mock host 通过 `scalar_hdv_ready_i/scalar_hdv_ep_done_i` 接管。

标量流水线应在 `hdv_scalar_valid_o` 有效且 `scalar_hdv_ready_i` 握手后接收一组并行标量指令 entry。每个 valid entry 都是一条完整指令。`scalar_hdv_ep_done_i` 表示标量后端已经执行并写回本 EP 的标量切片；真实后端是在 scalar slots 执行和写回完成后产生 done，mock host 则用固定 latency 产生它。

### vector pipeline 接口

顶层不再导出 `hdv_vector_*` 外部端口。HEU 向量侧在 `hdv_top` 内部连接到 `hdv_vec_dispatch_unit`，再由该模块驱动 Ara accelerator request。向量侧的 `vector_ep_acknowledged_i` 来自 `vec_ep_acknowledged_o`，语义是 vector EP 切片已被向量派发模块安全接管；vtrace 模式下普通 EP 入队即 acknowledged，真实标量模式下等本 EP 的 vector request 都 drain 并捕获 operand 后 acknowledged，`vset rd!=0` EP 还要等 response 写回后 acknowledged。
...
```text
scalar_axi_req/scalar_axi_resp <-> hdv_scalar_backend
```

当前 RTL 已经把 `hdv_scalar_backend` 的 load/store AXI 接入这个 slot。保留三路 `axi_mux` 拓扑的目的仍然是保持 Ara vector memory path、scalar memory path、HDV IPU 取指 path 共用一组 system AXI port，并保持 ID 宽度稳定。若后续标量 LSU 宽度与 system AXI 宽度不一致，可以在这个 slot 前补 `axi_dw_converter`。
...
- `scalar_axi_req/scalar_axi_resp`: `hdv_scalar_backend` 的标量 load/store path。
...
- 标量侧默认由 `hdv_top` 内部的 `hdv_scalar_backend` 接收；外部 `hdv_scalar_*` 端口仍导出给 `ara_tb` 观察或在关闭真实标量后端时交给 `hdv_mock_host_core` 接管。
...
HDV task CSR 在 `ara_soc` 中有两条来源：软件 `ctrl_registers` 产生的 CSR 访问，以及 testbench/mock host 直连的 host CSR 访问。host CSR 有效时优先送入 `hdv_top`。真实标量模式下，HDV scalar dispatch 在 `hdv_top` 内部由 `hdv_scalar_backend` 接收；`ara_tb` 中的 scalar 信号仍可观察。vector dispatch 在 `hdv_top` 内部通过 `hdv_vec_dispatch_unit` 转成 Ara accelerator request。
...
这些信号来自 HEU，只表示 execute packet 级“前端推进/错误”状态。当前 RTL 会把 `heu_top_ep_error` 和 `vec_ep_error` 并入 `task_error_to_tsu`，从而触发 task flush/error；`host_hdv_task_error_i` 仍是外部 task controller 可主动上报 task error 的入口。
...
| `heu_top_*` | HEU → hdv_top status | execute busy/acknowledged/error |
...
- 把 `hdv_scalar_backend` 扩展成覆盖全 Ara benchmark 的完整 RV64IMC + F/D + Zicsr 用户态标量通路。
- 让普通 Ara 指令退休/写回结果反向约束 EP acknowledged；当前只有 scalar 可见的 `vset rd!=0` response 会延迟 vector EP acknowledged。
...
| `acknowledged_eps_q` | [31:0] | MOCK 内部 | 已 acknowledged 的 execute packet 数 | 由 testcase/plusarg 决定 |
| `expected_ep_acknowledges_q` | [31:0] | MOCK 内部 | 期望 EP 数 | 可由 `+HDV_EXPECTED_EP=<N>` 覆盖 |
...
| `hdv_mock_ep_acknowledged_i` | 1 | HEU → MOCK | HEU 发来的 execute packet acknowledged 脉冲 | 每个前端可推进 EP 一次脉冲 |
...
| 5 | RUN | 等待并计数 ep_acknowledged 脉冲；若看到 HDV task done/busy 结束也可进入读状态 |
...
| `scalar_slice_outstanding_q` | 1 | HEU 内部 | 标量后端尚未 done | scalar_fire 后为 1 |
| `vector_slice_outstanding_q` | 1 | HEU 内部 | 向量后端尚未 acknowledged | vector_fire 后为 1 |
| `scalar_heu_ready_i` | 1 | scalar backend/mock → HEU | 标量后端 ready | 真实模式来自 `hdv_scalar_backend.scalar_ready_o` |
| `vector_heu_ready_i` | 1 | VEC_DISPATCH → HEU | 向量后端 ready | `hdv_vec_dispatch_unit` 空闲时为 1 |
| `scalar_ep_done_i` | 1 | scalar backend/mock → HEU | 标量后端完成本 EP 切片 | scalar slice 执行/写回完成后脉冲 |
| `vector_ep_acknowledged_i` | 1 | 向量后端 → HEU | 向量后端捕获本 EP 所需标量 operand | 普通 vector EP request drain 后脉冲；`vset rd!=0` 等 response 写回后脉冲 |
| `ep_acknowledged_q` | 1 | HEU 内部 | ep_acknowledged 脉冲（**1 cycle**） | 每个 EP 达到前端推进条件后出现，次周期自动清 0 |
| `heu_top_ep_acknowledged_o` | 1 | HEU → TOP/MOCK | = ep_acknowledged_q，连到 TOP.hdv_host_ep_acknowledged_o | 每个 acknowledged EP 一次脉冲 |
...
6. **看 `HEU.accept_packet` 和 `HEU.ep_acknowledged_q`**：确认 HEU 逐包接收，并在标量/向量后端 done/acknowledged 后发出 EP acknowledged 脉冲。
