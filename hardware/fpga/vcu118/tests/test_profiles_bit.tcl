...
proc write_bitstream {path} {
    ...
    if {$::case eq "native_drc"} { error "Vivado bitgen DRC failed" }
    put $path bitstream
}
...
foreach profile {baseline host dual_ddr} {
    ...
    foreach case {healthy wrong_profile drivers reports no_timing negative infinite native_drc} {
        set gates {}; set closed 0; set opened ""
        set out $scratch/${profile}_$case
        set argv [list $scratch/ara_dsa_vcu118_routed.dcp $out]
        set failed [catch {source $scripts/write_profile_bit.tcl} message]
        assert {$failed == ($case ne "healthy")} "$profile/$case: $message"
        assert {$closed == 1} "always close design, including gate failure"
        if {$case eq "healthy"} {
            assert {$gates eq {drivers reports bitgen}} "all gates before bitgen"
            ...
            if {$case ne "native_drc"} { assert {"bitgen" ni $gates} "gate failure blocks bitgen" }
        }
    }
}
unset ::env(ARA_FPGA_PROFILE)
puts "PASS: 24 profile bitgen gate scenarios (mocked, no synthesis)"
