...
def main(argv=None):
    ...
    if not args.program_confirmed:
        parser.error("--program-confirmed is required")
    output = args.out.resolve()
    output.mkdir(parents=True, exist_ok=False)
    report = {"mode": "restore" if args.restore else "diagnostic", "started_utc": datetime.now(timezone.utc).isoformat(), "state": "not_programmed"}
    code = 1
    try:
        ...
        print("PROGRAMMING", report["mode"], "EVIDENCE", output, flush=True)
        ...
        for line in console.splitlines():
            if line.startswith(("TARGET ", "PROGRAMMED ", "STATUS_", "MANAGEMENT_AXI ", "DIAGNOSTIC_BASELINE_PASS", "RESTORE_PROGRAMMED", "ETH_BOARD_ERROR ")):
        ...
        if run.returncode == 0 and ("RESTORE_PROGRAMMED" if args.restore else "DIAGNOSTIC_BASELINE_PASS") in console:
            ...
            report["state"] = "programmed_but_check_failed" if f"PROGRAMMED {report['mode']}" in console else "programming_failed_or_unknown"
