param(
    [string]$Port = 'COM6',
    [string]$Root = 'D:\qwen_small_combined_runs',
    [int]$Seconds = 30,
    [switch]$WaitForReset
)
...
$Summarizer = Join-Path $Software 'summarize_qwen_combined.py'
$Elf = Join-Path $Software 'qwen_small_combined\qbs_akv.elf'
if (-not (Test-Path -LiteralPath $Elf)) {
    throw "benchmark ELF not found: $Elf"
}

$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$Run = Join-Path $Root "qbs_akv_$Stamp"
...
& py -3 $Loader --port $Port --baud 115200 --elf $Elf --no-readback `
    --chunk-size 65536 --seconds $Seconds 2>&1 | Tee-Object -FilePath $Raw
