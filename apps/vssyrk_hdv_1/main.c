...
void ssyrk_f32_full(const float *A, float *C, int n, const float alpha,
    ...
    // Two-k inner unroll for the representative even N. Two independent
    // scalar/vector temporaries keep the two products distinct; the 14 useful
    // instructions fill two 256-bit packets exactly.
    ".balign 16\n"
    "k_loop:\n"
    "HDV_HINT 0x200, 1, 0, 1, 0\n"
    "flw ft0, 0(t4)\n"
    "fmul.s ft1, ft0, fa0\n"
    "vlse32.v v0, (t3), s1\n"
    "vfmacc.vf v4, ft1, v0\n"
    ...
    // Store the completed row, then update the two independent pointers. Zero
    // pbits keep the vector store and scalar updates in separate EPs.
    "HDV_HINT 0x00\n"
    "vse32.v v4, (t2)\n"
    "add t1, t1, s1\n"
    "add t2, t2, s1\n"

    // Keep the decrement-to-branch RAW cut while sharing one logical packet.
