...
// HDV-packetised softmax (channel-dim, Cephes exp). The dependent Cephes
// polynomial remains conservatively partitioned, while independent constant
// setup and broadcast work uses wider packets and cross-packet EP continuation.
...
void softmax_clean(const float *in, float *out, uint64_t ch, uint64_t inner) {
  __asm__ volatile (
    ...
    ".option norvc\n"
    ".option norelax\n"
    ".macro HDV_HINT pbits=0x00, packet256=0, cross=0, loop_start=0, loop_end=0, prefetch_mode=0, prefetch_disable=1\n"
    ...
    // la expands to two instructions. Together with five scalar loads it fills
    // one 256-bit packet; the remaining constants fill three 128-bit packets.
    "HDV_HINT 0x00, 1\n"
    ...
    "vfmv.v.f v17, fa0\n"
    "vfmv.v.f v18, ft0\n"
    // Six independent broadcasts plus the first pointer reset occupy one
    // 256-bit packet. Cross carries its three-instruction tail into the next
    // packet so the first EP after the issue-width cut is also full.
    "HDV_HINT 0xaaa, 1, 1\n"
    "vfmv.v.f v19, ft1\n"
    "vfmv.v.f v20, ft2\n"
    "vmv.v.x  v21, t1\n"
    ...
    // Polynomial tail, store, reduction update, pointer updates and branch:
    // seven useful instructions in one 256-bit packet.
    "HDV_HINT 0x00, 1, 0, 0, 1\n"
