...
void fconv2d_clean(double *o, double *in, double *flt, int64_t R, int64_t C,
    ...
    // VL, filter coefficients, strides and bases. Two 256-bit packets plus
    // two 128-bit packets carry the 20 setup instructions without padding.
    "HDV_HINT 0x02, 1, 0\n"
    ...
    // Output-row loop. The 31 useful instructions occupy four 256-bit packets
    // and one 128-bit packet exactly, so the repeated body has no padding NOPs.
