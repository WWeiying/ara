...
void gemv_f32(const float *A, const float *x, float *y, int n, int groups) {
    ...
    // Setup fits one 256-bit packet. The argument registers themselves serve as
    // the running A/y pointers, avoiding a separate pointer-alias packet.
    "HDV_HINT 0x280, 1\n"
    ...
    ".balign 16\n"
    "row_loop:\n"
    // Eleven useful row instructions use one 256-bit packet and two 128-bit
    // packets. Only the branch-only packet retains two padding slots.
    "HDV_HINT 0x022, 1, 0, 1, 0, " BL_STR(BL_PFM) "\n"
    "vsetvli s0, a3, e32, m" BL_STR(BLAS_LMUL) ", ta, ma\n"
    "vle32.v v" BL_STR(BL_G1) ", (a0)\n"
    ...
    "HDV_HINT 0x0a\n"
    "addi a0, a0, " BL_STR(BLAS_LMUL) "*128\n"
    "addi a2, a2, 4\n"
    "addi t0, t0, -1\n"
