...
void strsm_f32_left_lower(const float *L, float *B, int m) {
    ...
    // Setup has seven useful instructions. Keep the vset dependency cuts in
    // pbits while using one 256-bit logical packet with no padding.
    "HDV_HINT 0x00, 1\n"
    ...
    // Keep scalar address/load and vector load/compute/store in separate
    // logical packets. These gaps prevent the next scalar load from overtaking
    // an in-flight vector store through the cross-domain memory-order guard.
    ".balign 16\n"
    "update_loop:\n"
