...
def validate_spec(spec: dict) -> None:
    limits = spec["limits"]
    context = spec["activation_context"]
    ...
    if not 1 <= context["count"] <= 15:
        raise ValueError("activation_context.count must fit the 4-bit field")
    if not 1 <= context["max_m"] <= limits["max_m"]:
        raise ValueError("activation_context.max_m exceeds the QBS M limit")
    if not 1 <= context["max_k_blocks"] <= min(
            limits["max_k_blocks"], 32):
        raise ValueError(
            "activation_context.max_k_blocks must fit the capability field")
    if context["generation_bits"] != 8:
        raise ValueError("descriptor v2 requires an 8-bit context generation")
    ...
    for name in context["activation_profiles"]:
        if name not in spec["activation_profiles"]:
            raise ValueError(
                f"activation context references unknown profile {name}")
    for name in context["activation_layouts"]:
        if name not in spec["activation_layouts"]:
            raise ValueError(
                f"activation context references unknown layout {name}")
...
def c_header(spec: dict) -> str:
    ...
    context_profile_mask = " |\n        ".join(
        f"(UINT64_C(1) << QBS_ACTIVATION_PROFILE_{name})"
        for name in context["activation_profiles"])
    context_layout_mask = " |\n        ".join(
        f"(UINT64_C(1) << QBS_ACTIVATION_LAYOUT_{name})"
        for name in context["activation_layouts"])
...
#define QBS_ACTIVATION_CONTEXT_COUNT {context['count']}u
#define QBS_ACTIVATION_CONTEXT_MAX_M {context['max_m']}u
#define QBS_ACTIVATION_CONTEXT_MAX_K_BLOCKS {context['max_k_blocks']}u
#define QBS_ACTIVATION_CONTEXT_GENERATION_BITS {context['generation_bits']}u
...
typedef enum {{
  QBS_ACTIVATION_ACCESS_DIRECT = {ACTIVATION_ACCESS_MODES['DIRECT']},
  QBS_ACTIVATION_ACCESS_FILL = {ACTIVATION_ACCESS_MODES['FILL']},
  QBS_ACTIVATION_ACCESS_REUSE = {ACTIVATION_ACCESS_MODES['REUSE']},
  QBS_ACTIVATION_ACCESS_RELEASE = {ACTIVATION_ACCESS_MODES['RELEASE']},
...
`endif
  ...
  localparam int unsigned QbsActivationContextCount = {context['count']};
  localparam int unsigned QbsActivationContextMaxM = {context['max_m']};
  localparam int unsigned QbsActivationContextMaxKBlocks = {context['max_k_blocks']};
  localparam int unsigned QbsActivationContextGenerationBits = {context['generation_bits']};
...
{sv_layout_enum(spec, 'ACTIVATION')}
  ...
  typedef enum logic [1:0] {{
    QBS_ACTIVATION_ACCESS_DIRECT = 2'd{ACTIVATION_ACCESS_MODES['DIRECT']},
    QBS_ACTIVATION_ACCESS_FILL = 2'd{ACTIVATION_ACCESS_MODES['FILL']},
    QBS_ACTIVATION_ACCESS_REUSE = 2'd{ACTIVATION_ACCESS_MODES['REUSE']},
    QBS_ACTIVATION_ACCESS_RELEASE = 2'd{ACTIVATION_ACCESS_MODES['RELEASE']}
...
{chr(10).join(f"        result[12 + QBS_ACTIVATION_PROFILE_{name}] = 1'b1;" for name in context['activation_profiles'])}
        result[31:28] = 4'b1111;
        result[39:32] = 8'(QbsActivationContextGenerationBits);
{chr(10).join(f"        result[40 + QBS_ACTIVATION_LAYOUT_{name}] = 1'b1;" for name in context['activation_layouts'])}
      end
